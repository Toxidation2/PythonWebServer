            ...
            if os.path.isfile(htDocsDir + self.path.split("?")[0]):
                ...
                if newContentType[0:2] == "py":
                    if newBoolean:
                        old_stdout = sys.stdout
                        sys.stdout = stdout = StringIO()
                        file = open(htDocsDir + self.path.split("?")[0], "rb")
                        content = file.read()
                        file.close()
                        exec(content)
                        sys.stdout = old_stdout
